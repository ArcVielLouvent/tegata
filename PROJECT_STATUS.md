...
**Status: unblocked.** Next: re-run `scripts/verify_doctavian_template.py` (now uses `upload_template()` + direct `template_urn`, no `create_template()` call) with a fresh access token, and finally confirm whether the native Word IF field renders differently for high vs. low risk — this is the one remaining unverified assumption in Phase 2.
